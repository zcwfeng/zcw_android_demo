...
@SuppressLint("NewApi")
class SimpleColorChangeView2 @JvmOverloads constructor(context: Context, attrs: AttributeSet, defStyleAttr: Int=0, defStyleRes: Int=0)
    ...
    init {
        ...
        Log.e(TAG, "init mLinePaint")
    }

    init {
        val ta: TypedArray = context.obtainStyledAttributes(attrs, R.styleable.SimpleColorChangeView2)
        text = ta.getString(R.styleable.SimpleColorChangeView2_text) as String
        textSize = ta.getDimensionPixelSize(R.styleable.SimpleColorChangeView2_text_size, textSize.toInt()).toFloat()
        textColor = ta.getColor(
                R.styleable.SimpleColorChangeView2_text_color, textColor)
        textColorChange = ta.getColor(
                R.styleable.SimpleColorChangeView2_text_color_change, textColorChange)
        progress = ta.getFloat(R.styleable.SimpleColorChangeView2_progress, 0f)
        direction = ta
                .getInt(R.styleable.SimpleColorChangeView2_direction, direction)
        ta.recycle()
...
//                canvas.drawRect((int) (mTextStartX + (1 - mProgress) * mTextWidth), 0, mTextStartX + mTextWidth, getMeasuredHeight(), mLinePaint);
                drawTextHorizontal(canvas, textColorChange,
                        (mTextStartX + (1 - progress) * mTextWidth), (mTextStartX + mTextWidth).toFloat())
                //后绘制没改变的
                //后绘制没改变的
                drawTextHorizontal(canvas, textColor, mTextStartX.toFloat(),
                        (mTextStartX + (1 - progress) * mTextWidth))
            }
            DIRECTION_TOP -> {
                ...
                //先绘制改变的颜色
                //先绘制改变的颜色
                drawTextVertical(canvas!!, textColorChange, mTextStartY.toFloat(),
                        ((mTextStartY + progress * mTextHeight) as Int).toFloat())
                //后绘制没改变的

                //后绘制没改变的
                drawTextVertical(canvas, textColor,
                        ((mTextStartY + progress * mTextHeight) as Int).toFloat(), (mTextStartY + mTextHeight).toFloat())
            ...
            DIRECTION_BOTTOM -> {
                ...
                drawTextVertical(canvas!!, textColorChange,
                        ((mTextStartY + (1 - progress) * mTextHeight) as Int).toFloat(), (mTextStartY + mTextHeight).toFloat())
                //后绘制没改变的
                //后绘制没改变的
                drawTextVertical(canvas, textColor, mTextStartY.toFloat(),
                        ((mTextStartY + (1 - progress) * mTextHeight) as Int).toFloat())
            }
        }
    }

    private fun drawTextVertical(canvas: Canvas, color: Int, startY: Float, endY: Float) {
        mPaint.color = color
        canvas.save()
        canvas.clipRect(0, startY.toInt(), measuredWidth, endY.toInt())
        canvas.drawText(text, mTextStartX.toFloat(), measuredHeight / 2
                - (mPaint.descent() + mPaint.ascent()) / 2, mPaint)
        canvas.restore()
    }

    private fun drawTextHorizontal(canvas: Canvas?, color: Int, startX: Float, endX: Float) {
        mPaint.color = color
        canvas?.save()
        canvas?.clipRect(startX.toInt(), 0, endX.toInt(), measuredHeight)
        canvas?.drawText(text, mTextStartX.toFloat(), measuredHeight / 2
                - (mPaint.descent() + mPaint.ascent()) / 2, mPaint)
        canvas?.restore()
    }
