...
public class FishheaderView extends View {
	...
	private static final int BODY_ALPHA = 220;//身体透明度
	private static final int OTHER_ALPHA = 160;//其他部分透明度
	private static final int FINS_ALPHA = 100;//鱼鳍透明度
	private static final int FINS_LEFT = 1;//左鱼鳍
	private static final int FINS_RIGHT = -1;//右鱼鳍
	private static final float FINS_LENGTH = HEAD_RADIUS * 1.3f;//鱼鳍长度
	private Paint mPaint;
	private Context mContext;
	//控制区域
	private int currentValue = 0;//全局控制标志
//	private float mainAngle = new Random().nextFloat() * 360;//角度表示的角
private float mainAngle = 0f * 360;//角度表示的角
	private float waveFrequence = 0;
	//鱼头点
	...
	public FishheaderView(final Context context) {
		this(context,null);
	}

	public FishheaderView(final Context context, @Nullable final AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}
	...
	private void init() {
		...
		mPaint.setColor(Color.argb(OTHER_ALPHA, 71, 92, 244));
		//身体画笔
		bodyPaint = new Paint();
		bodyPaint.setAntiAlias(true);
		bodyPaint.setStyle(Paint.Style.FILL);
		bodyPaint.setDither(true);//防抖
		bodyPaint.setColor(Color.argb(OTHER_ALPHA + 5, 71, 92, 244));
		//骨架画笔
		mLinePaint = new Paint();
		mLinePaint.setAntiAlias(true);
		mLinePaint.setStyle(Paint.Style.FILL_AND_STROKE);
		mLinePaint.setStrokeWidth(15);
		mLinePaint.setDither(true);//防抖
...
//		makeSegments(canvas, mainPoint, headRadius * 0.7f, 0.6f, angle);
		...
		mLinePaint.setColor(Color.RED);
		canvas.drawPoint(point1.x,point1.y,mLinePaint);
		canvas.drawPoint(point2.x,point2.y,mLinePaint);
		canvas.drawPoint(point3.x,point3.y,mLinePaint);
		canvas.drawPoint(point4.x,point4.y,mLinePaint);
		//决定胖瘦
...
//		canvas.drawPoint(contralLeft.x,contralLeft.y,mLinePaint);
//		canvas.drawPoint(contralRight.x,contralRight.y,mLinePaint);
		mPath.reset();
		...
		mPaint.setColor(Color.argb(BODY_ALPHA, 71, 92, 244));
		//画最大的身子
	...
	private void makeFins(Canvas canvas, PointF startPoint, int type, float fatherAngle) {
		...
		mPaint.setColor(Color.argb(FINS_ALPHA, 71, 92, 244));
		canvas.drawPath(mPath, mPaint);
		mPaint.setColor(Color.argb(OTHER_ALPHA, 71, 92, 244));
