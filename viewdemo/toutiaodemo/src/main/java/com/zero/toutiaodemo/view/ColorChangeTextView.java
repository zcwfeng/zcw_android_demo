...
public class ColorChangeTextView extends View {
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        Log.i(TAG, "fontMetrics = (" + fontMetrics.top
        + " - " + fontMetrics.ascent + ", " + fontMetrics.leading
        + ", " + fontMetrics.descent + " - " +fontMetrics.bottom);
        mTextHeight = (int)(fontMetrics.descent - fontMetrics.ascent + .5f);
        Log.i(TAG, "mTextBound.height = " + mTextBound.height() + " , height = " + (mTextBound.top + mTextBound.bottom) + " , bounds = " + mTextBound);

        mTextStartX = getMeasuredWidth() / 2 - mTextWidth / 2;
        mTextStartY = getMeasuredHeight() / 2 - mTextHeight / 2;
        mBaseLine = getMeasuredHeight() / 2 - ((mPaint.descent() + mPaint.ascent()) / 2);
        Log.i(TAG, "mBaseLine = " + mBaseLine);
        mBaseLine = getMeasuredHeight() / 2 - (mTextBound.top + mTextBound.bottom) / 2;
        Log.i(TAG, "mBaseLine2 = " + mBaseLine);
        Log.i(TAG, "mTextStartY = " + mTextStartY + " , mTextStartX = " + mTextStartX);
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        switch (mDirection) {
            case DIRECTION_LEFT:
                ...
                canvas.drawText(mText,mTextStartX,mBaseLine,mPaint);
                canvas.restore();
                //2.绘制没有改变 底色
